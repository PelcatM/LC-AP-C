...
public class ExampleBluetoothApp {
		...
		try (DataOutputStream dataOS = connection.openDataOutputStream()) {		
			try (DataInputStream dataIS = connection.openDataInputStream()) {	
				...
				// Write anything you want : can also wrap stream on Object-Input/Output-Stream, or use XML/JSON in UTF String.
					...
					dataOS.write(bytes);	      //Requete
					dataIS.read(reponse);		  //Response
					Commandes.response(reponse);  //appel de la class commande
			}
		}
	}
	...
	public void launchExample() throws Exception {
		...
		String serviceURL = searchService(selected);  //Recherche l'application sur un device
		LOGGER.info("Found the wanted service with URL {}", serviceURL);
		
		byte[] requeste = Commandes.writeCommande("send");
		
		//String lastring = "Le tableau = ";
		//for(int i = 0; i < requeste.length; i++) {
		//	  lastring = lastring + requeste[i] +" ,";
		 // }
		//System.out.println(lastring); 
		executeBytesRequest(requeste, serviceURL);		//Envoi le message en bytes
